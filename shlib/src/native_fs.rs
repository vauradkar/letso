...
impl DirWalker {
    ...
    pub async fn walk_dir<P: AsRef<Path>>(
        ...
        x.await.map_err(|e| Error::ReadError {
    ...
    async fn write_chunks(&self, chunks: &mut Vec<SyncItem>) -> Result<(), Error> {
        self.tx
    ...
    async fn push_and_send(&self, chunks: &mut Vec<SyncItem>, item: SyncItem) -> Result<(), Error> {
        chunks.push(item);
    ...
    pub async fn walk_dir_stream<P: AsRef<Path>>(&self, full_path: &P) -> Result<(), Error> {
